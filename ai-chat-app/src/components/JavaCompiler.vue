<template>
  <div v-if="visible" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-11/12 max-w-4xl p-6 relative">
      ...
      <h2 class="text-2xl font-bold mb-4 text-gray-800">Java Compiler</h2>
...
export default {
  ...
  methods: {
    async compileAndRun() {
      try {
        const response = await axios.post('/compile_run_java', { code: this.code });
...
<style scoped>
.code-background {
  background-image: url('./pic/Java.png');
